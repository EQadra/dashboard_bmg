...
export const useUsersStore = defineStore('users', () => {
  ...
  /** Asignar rol a usuario (si tuvieras endpoint para esto) */
  const assignRole = async (userId, roleId) => {
    await getCsrfToken();
    try {
      const res = await api.post(`/api/usuarios/${userId}/rol`, { role_id: roleId });
      const updated = res.data?.data ?? res.data;
      const user = users.value.find((u) => u.id === userId);
      if (user) {
        user.roles = updated?.roles ?? (updated?.role ? [updated.role] : [{ id: roleId }]);
      }
      return updated;
    } catch (err) {
